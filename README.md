...
| `package`         | package | _(backend, built from source)_ | The Polyfloor package to run. The default builds the **backend only** (API + SPA-if-`staticDir`-set). Use `polyfloor-full` to bundle the built frontend SPA. |
...
| `environmentFile` | path    | null                           | Path to an env file with secrets (sops). Never put secrets in the module config.                                                                             |
...
| `staticDir`       | path    | null                           | Directory of the built frontend SPA to serve (used by `nix run` / `polyfloor-full`).                                                                         |
